...
void retrieve(lineitem** l_tups) {
	const uint64_t total_size = MAX_TUPLES * TUPLE_SIZE;
	...
	* l_tups = (lineitem*)malloc(sizeof(lineitem));
	...
	uint32_t total_heap_loc = MAX_TUPLES * sizeof((*l_tups)->l_shipdate[0]);
	(*l_tups)->l_discount = (uint64_t*)(tuple_data + total_heap_loc);
	total_heap_loc += MAX_TUPLES * sizeof((*l_tups)->l_discount[0]);
	(*l_tups)->l_quantity = (uint64_t*)(tuple_data + total_heap_loc);
	total_heap_loc += MAX_TUPLES * sizeof((*l_tups)->l_quantity[0]);
	...
	if (!access(db_bin.c_str(), F_OK)) {
		...
		if (!fread((void*)tuple_data, bytes_read, 1, f_bin)) {
			std::cerr << "Error: mismatching tuples in database file." << std::endl;
			exit(12);
		}
		fclose(f_bin);
		(*l_tups)->elements = bytes_read / TUPLE_SIZE;
		return;
	}
